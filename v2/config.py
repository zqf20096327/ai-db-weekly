...
# 互斥去重仅用于 database 桶：旧版四方互斥会把同时打多个标签的项目全部漏采
# （如 Chat2DB 同时有 database+mysql+postgresql+oracle 标签，被四个查询同时排除）。
# 现在 database 排除三个具体库（泛库桶避免重复抓大头），具体库之间不互斥，
# 跨 topic 重复由 storage.merge_dedupe 按 full_name 去重。
...
# 仅 database 做互斥（减号语法；GitHub 不支持 NOT，会返回 422）
MUTUAL_EXCLUDE = {
    "database": " ".join(f"-topic:{t}" for t in BIG_FOUR if t != "database"),
}
...
# ============================================================
# 白名单固定追踪 repo（SOP 5.1 —— 内核补采 + 重要生态项目漏采兜底）
# ⚠️ 必须手工确认是官方源而非镜像/撞名
# ============================================================
WHITELIST_REPOS = [
    # 国际主流关系库
...
KERNEL_REPOS = {
    ...
    # NewSQL / 分布式
    "pingcap/tidb", "tikv/tikv", "pingcap/tiflash", "oceanbase/oceanbase",
...
# ----- §3.1 三板块归属关键词（新口径：范围外排斥 + AI 优先 > 国产 > 国外）-----
# 周报板块范围（用户口径 2026-08-14）：
#   板块一 · 国际主流：Oracle / SQL Server / DB2 / MySQL / PostgreSQL / MariaDB / ClickHouse
#   板块二 · 国产：openGauss / GaussDB / TiDB / OceanBase / TDSQL / PolarDB / PolarDB-X /
#                   YashanDB / GBase / DM / GoldenDB
#   板块三 · AI 工具：上述范围内数据库生态的 AI 辅助工具
SCOPE_INTL_DISPLAY = "Oracle / SQL Server / DB2 / MySQL / PostgreSQL / MariaDB / ClickHouse"
SCOPE_CN_DISPLAY = (
    "openGauss / GaussDB / TiDB / OceanBase / TDSQL / PolarDB / PolarDB-X / "
    "YashanDB / GBase / DM / GoldenDB"
)

# 范围内产品名（供板块归属 + AI 板块范围复核 + 适用数据库推断复用）
SCOPE_INTL_DB_NAMES = [
    "oracle", "mysql", "mariadb", "postgresql", "postgres",
    "sqlserver", "sql server", "mssql", "db2", "clickhouse",
]
SCOPE_CN_DB_NAMES = [
    "opengauss", "gaussdb", "tidb", "tikv", "oceanbase", "tdsql",
    "polardb", "polardb-x", "polardbx", "yashandb", "yashan",
    "goldendb", "gbase", "dameng", "dm8", "dm database", "dm",
]
# 注：裸 "dm" 走独立 token 匹配（admin/odm 等是不同 token，不会误命中），
#     用于识别 GitHub topic:dm 的达梦生态项目。
SCOPE_DB_NAME_KEYWORDS = SCOPE_INTL_DB_NAMES + SCOPE_CN_DB_NAMES

SECTION_KEYWORDS = {
    # 国产 = 范围内国产产品名（原 doris/starrocks 不在范围清单内，移入范围外排斥）
    "国产数据库": list(SCOPE_CN_DB_NAMES),
    # 国外 = 范围内国外产品名（准入必须命中具体库名；泛词"database/sql"不再
    #        作为兜底 —— SawitDB/discodb 这类独立实验引擎不入板块。候选层
    #        再用 README 做范围复核，见 run_weekly.stage_select）
    "国外数据库": list(SCOPE_INTL_DB_NAMES),
}

# ----- 范围外数据库排斥清单（板块范围闸门）-----
# 口径：不属于板块范围的数据库，禁止放入对应板块 → 命中任一即整项目不进周报
# （三板块 + Top 总榜同一套闸门）。子串匹配；清单内词在数据库语境中独特，
# 不误命中无关词。发现新的范围外库生态项目混入时按需追加。
OUT_OF_SCOPE_DB_KEYWORDS = [
    # 文档 / KV / NoSQL
    ...
    # NewSQL / 其他（范围外）
    "cockroach", "yugabyte", "singlestore", "memsql",
    "surrealdb", "sqlite", "libsql", "turso",
    # 文件型 / 通用存储伪数据库（泛词残留桶兜底，如 lowdb）
...
# ----- §3.5 适用数据库推断（仅范围内 18 库，范围外库名不再出现在周报卡片）-----
...
APPLICABLE_DB_KEYWORDS = {
    ...
    # 板块二 · 国产
    "openGauss": ["opengauss", "open-gauss", "open gauss"],
    "GaussDB": ["gaussdb", "gauss db"],
    "TiDB": ["tidb", "tikv"],
    "OceanBase": ["oceanbase"],
    "TDSQL": ["tdsql"],
    "PolarDB": ["polardb"],
    "PolarDB-X": ["polardb-x", "polardbx"],
    "YashanDB": ["yashandb", "yashan"],
    "GBase": ["gbase"],
    "DM": ["dameng", "dm database", "dm-database", "dm8"],
    "GoldenDB": ["goldendb"],
}
...
SOP_EXCLUDE_KEYWORDS = {
    ...
    "app_layer": ["cms", "blog", "e-commerce", "ecommerce", "admin panel",
                   ...
                   # 终端用户应用（仅把范围内库当后端技术栈，非数据库生态工具）
                   "management app", "management-app",
                   # 垂直应用平台强信号（WordPress 插件/电商建站等，运行底座固定为某库，
                   # 但生态归属是平台而非数据库工具）
...
NEWSTAR_TOP_N = 3          # 新锐发现 TopN（最多展示 3 个）
TOPBOARD_TOP = 5           # Top 总榜 TopN（历史 star 总数）
BRIEF_MAX_CHARS = 80       # AI 解读字数上限（中性、客观、无绝对词）
FOCUS_SEG_MAX_CHARS = 100  # 本周解读三段（解决什么/核心亮点/使用场景）单段上限
# 候选池超额选取条数：compute 多选一些，README 范围复核剔除后仍能递补满 TopN
