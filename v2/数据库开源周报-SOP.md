...
> 📌 **保留策略（2026-09-20 定稿）**：快照永久保留，不再滚动清理——近 8 天保持明文目录（周报 7 天 diff 依赖目录结构），出窗快照由 `v2/snapshot_retention.py` 压缩为 `data/snapshot_YYYYMMDD.zip` 永久入库（约 33MB→5MB/天，含 CI 每日自动转档）。原因：GitHub API 只返回当前状态、历史 star/forks 无法回填，快照即不可再生的数据资产，是长期"数据湖"（月度/年度趋势分析）的地基。
...
### 6.z 跨期轮换（2026-09-19 定稿，防相邻期重复）

**问题**：6.y 只解决单期内的项目霸屏；跨期看，持续高增长项目（如 bytebase/dbx）会连周占据活跃榜卡片与「本周解读」，读者翻来覆去看同一批项目。

**规则（栏目结构不变，只改"谁上榜"）**：

1. **活跃榜排他（软约束）**：剔除「最近 K 期」已上榜项目后按本周 star 净增取 TopN（`config.ACTIVE_COOLDOWN_ISSUES`，当前 K=1 即隔期轮换，霸榜者每 K+1 期回归一次）。剔除后凑不满 TopN 时按净增顺序放回补位——**足额优先于轮换**，不捧弱项目。
2. **爆发豁免**：项目本周净增 ≥ 上次上榜当期净增 × 2（`ACTIVE_GROWTH_EXEMPT_RATIO`）时可跳过冷却直接回归（持续爆发是真新闻，冷却不压）。
3. **解读冷却（长于榜单）**：最近 4 期（`FOCUS_COOLDOWN_ISSUES`）做过「本周解读」的项目不再入选解读对象（豁免同上）；板块候选全员冷却时取活跃榜榜首兜底。设计意图：**数据可常见（榜单冷却短），深度须少见（解读冷却长）**。
4. **新锐发现 / Top 总榜不适用**：新锐按定义 7 天内创建、跨期不可能重复；总榜是背景板，头部常驻是其本性。
5. **跨期状态**：`reports/weekly_history.json`（追加式索引，每期一条：各板块上榜名单 + 解读对象 + 当期净增；快照出窗后转 zip 永久归档但不再直接可读，定榜历史以本索引为准）。渲染成功后写入，同日重跑覆盖；文件缺失时从磁盘残留定榜结果按期号去重自举。
6. **口径透明**：轮换口径不在周报正文披露（版式与创刊期保持一致），规则记录于本节 §6.z 与 config.py 注释；上榜语义随之从"本周最活跃 TopN"变为"上期未上榜的活跃 TopN"，此为编辑取舍——新鲜度优先于字面热度，头部项目仍由总榜呈现。
...
周报渲染阶段（run_weekly.py --only render）自 2026-09-07 第 1 期起同时产出两份文件：
...
2. **手机单栏固定布局**：正文 15px、内容区 max-width 677px（公众号正文标准宽度），色板/卡片样式沿用第 1 期手工定稿版（见 `wechat.py` 顶部设计令牌，改版需同步本附录）。
...
| v1.2 | 2026-09-19 | 新增 §6.z 跨期轮换：活跃榜隔期排他（K=1，软约束足额兜底，净增 2 倍豁免）+ 解读 4 期冷却；跨期状态落 reports/weekly_history.json |
| v1.3 | 2026-09-19 | 期号改制：对外正式创刊日 2026-09-07 = 第 1 期（2026-08 的 0817/0824/0831 为试刊不占期号），config.FIRST_ISSUE_DATE 同步；周报产物（report/wechat）渲染后统一自动归档至 reports/ |
| v1.4 | 2026-09-20 | 快照保留策略改为永久数据湖（§4.6）：近 8 天明文 + 出窗 zip 归档入库，替代"滚动 8 天清理"；执行者 v2/snapshot_retention.py（本地补档 + CI 每日转档，见 v2-daily.yml 步骤 6）。GitHub 历史无法回填，快照即数据资产 |
