...
## 附录 C：公众号版产物（weekly/wechat.html）

周报渲染阶段（run_weekly.py --only render）自 2026-09-07 第 5 期起同时产出两份文件：

- `weekly/report.md`：GitHub 展示用 Markdown 周报
- `weekly/wechat.html`：公众号专用 HTML（渲染器 `wechat.py`，随数据自动生成）

公众号编辑器会剥掉 `<style>`/class/媒体查询等非内联样式，因此公众号版约定：

1. **全内联样式**：只用 `<section>/<p>` + `style` 属性（编辑器保留度最高），不写 `<style>` 标签、class、hover/media query。
2. **手机单栏固定布局**：正文 15px、内容区 max-width 677px（公众号正文标准宽度），色板/卡片样式沿用第 5 期手工定稿版（见 `wechat.py` 顶部设计令牌，改版需同步本附录）。
3. **Top 总榜用卡片列表**，不用 `<table>`（表格在公众号手机端显示差且常被编辑器改写）。
4. **GitHub 链接以纯文本呈现**：普通公众号正文不允许外链，`<a>` 会被过滤；如账号有"文中链接"权限可再自行加回。
5. **发布方式**：浏览器打开 wechat.html → Ctrl+A 全选复制 → 粘贴进公众号编辑器 → 手机预览确认后发布。
6. **长描述截断**：卡片内描述 100 字 / 总榜 70 字，避免长英文 description 撑爆手机卡片。
...
| v1.1 | 2026-09-08 | 渲染阶段新增公众号版产物 weekly/wechat.html（附录 C）；三板块周报正文见 README_v2 |
